...
import { revealSecretData, updateSecretData } from "@/app/hr/self-service/actions"
...
export function PersonalDataCard() {
  ...
  const handleReveal = async () => {
    ...
    setLoading(true)
    const result = await revealSecretData()
    setLoading(false)
  ...
  const handleUpdate = async (formData: FormData) => {
    setLoading(true)
    const result = await updateSecretData(formData)
    setLoading(false)
      ...
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <div className="space-y-1">
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            <Lock className="w-4 h-4 text-destructive" /> Különleges Adatok
          </CardTitle>
          <CardDescription>Szigorúan bizalmas, auditált adatok</CardDescription>
        </div>
        ...
        <div className="mt-4 space-y-4">
          ...
          {!isRevealed && (
            <div className="bg-muted/50 p-2 rounded-md text-xs text-muted-foreground flex items-start gap-2 mt-4">
              ...
              <p>A fenti adatok megtekintése GDPR hatálya alá esik, és a rendszer a hozzáférést a központi Eseménynaplóban visszavonhatatlanul rögzíti.</p>
            </div>
          )}
        </div>
      </CardContent>
