...
CREATE POLICY "Mindenki lathatja a helyettesiteseket" ON helyettesites FOR SELECT TO authenticated USING (true);
...
CREATE TABLE irat_fizikai_hely (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE irat_kolcsonzes_naplo (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE POLICY "Kolcsonzesek megtekintese" ON irat_kolcsonzes_naplo FOR SELECT TO authenticated USING (true);
