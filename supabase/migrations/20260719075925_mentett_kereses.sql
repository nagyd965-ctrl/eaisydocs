CREATE TABLE IF NOT EXISTS mentett_kereses (
...
DROP POLICY IF EXISTS "Mentett keresések megtekintése" ON mentett_kereses;
CREATE POLICY "Mentett keresések megtekintése" ON mentett_kereses
FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Mentett keresések létrehozása" ON mentett_kereses;
CREATE POLICY "Mentett keresések létrehozása" ON mentett_kereses
FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Mentett keresések módosítása" ON mentett_kereses;
CREATE POLICY "Mentett keresések módosítása" ON mentett_kereses
FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Mentett keresések törlése" ON mentett_kereses;
